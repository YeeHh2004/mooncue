...
5. 播放器曲目目录与元数据继承、源音频排除区间审计、UTF-8 管道和批量检查。
6. 命令行入口、跨平台 CI、测试、示例、参考来源和使用文档。
...
项目采用 Apache-2.0；独立实现，不复制上述项目源代码。开发使用 AI 辅助，参赛者需理解接口、间隙策略、测试及已知限制。首版已实现上列能力，本地 47 项核心测试在 JS/Wasm 两个目标各通过一次，31 项接口/CLI 测试通过；尚未报名或取得主办方审核结论。

外部测试数据：引用 [MaxMEllon/cue-parser](https://github.com/MaxMEllon/cue-parser) 的两份 MIT 许可 CUE 文本，已保留来源与许可证；未复制其解析器代码。开发提交说明见 [COMMIT-AUDIT.md](COMMIT-AUDIT.md)。
